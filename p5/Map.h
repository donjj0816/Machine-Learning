...
private:
  ...
  class PairComp {
    ...
    private:
      Key_compare compares;

    public:
      bool operator() (Pair_type left, Pair_type right) const {
        return compares(left.first, right.first);
      }
  };
...
public:
  ...
  Iterator find(const Key_type& k) const{
    std::pair<Key_type, Value_type> place_holder;
    place_holder.first = k;
    place_holder.second = Value_type();
    return stats.find(place_holder);
  }
  ...
  Value_type& operator[](const Key_type& k){
    std::pair<Iterator,bool> holder = insert({k,Value_type()});
    Pair_type &holder_p = *(holder.first);
    return holder_p.second;
  ...
  std::pair<Iterator, bool> insert(const Pair_type &val){
    ...
    if(find(val.first) == stats.end()) {
        return std::pair<Iterator, bool> {stats.find(val), false};
    } else {
        return std::pair<Iterator, bool> {stats.find(val), true};
...
private:
BinarySearchTree<Pair_type, PairComp> stats;
};
