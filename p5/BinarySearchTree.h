...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static int height_impl(const Node *node) {
    ...
    if (node == nullptr) {
      ...
      first = height_impl(node->left);
      second = height_impl(node->right);
      if (first > second) {
  ...
  static Node *copy_nodes_impl(Node *node) {
    if (node == nullptr){
      ...
      holder->right = copy_nodes_impl(node->right);
      holder->left = copy_nodes_impl(node->left);
      holder->datum = node->datum;
      return holder;
    }
  }
